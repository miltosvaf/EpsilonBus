CREATE PROCEDURE [dbo].[getStopsActiveDetails]@BranchID int, @LanguageID int=1
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT s.ID AS StopID, s.StopCode, isnull(tt.TranslatedText, s.StopName) AS StopName, s.Latitude, s.Longitude
	FROM Stops s
